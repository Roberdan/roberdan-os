...
- **CI** (`test/validate.sh`) — frontmatter, links, deterministic generation, shellcheck,
  leak-check, kb/factory/federation/receipts/install regression, fork-merge proof.
...
- **Self-improving meta-loop** (`learn/` → `ontology/`): captures daily but the distill step writes
  a `TODO` classification stub, so **zero knowledge has been promoted automatically**; most captured
  "signal" is a boilerplate session line. This is the biggest prose-vs-reality gap.
...
| Agents (8) | `agents/` — thor, twin, baccio, rex, luca, socrates, board, wanda | real subagents on Claude Code; prose personas on other tools |
...
| Hooks | `hooks/` — bash-guard, context-inject, auto-checkpoint, autofmt, verify-done, main-guard, post-task-sync, pre-commit | mixed — see the honest map above for which fire |
...
| Meta-loop | `learn/` + `ontology/` + `evolve/` | scaffolding (see honest map) |
...
| Per-platform wrappers | `platforms/` — generated by `bin/sync.sh --emit-only`, gitignored, never committed | real |
...
- **Cross-tool parity is partial.** `AGENTS.md` behavior propagates everywhere, but *subagents* and
  *hooks* are Claude-Code-specific; on Copilot/Codex the 8 agents are prose personas, not invokable.
- **Full function depends on external, personal tooling** (the gbrain fork, gstack). Without them,
  recall and some skills degrade.
- **The meta-loop and evolve don't yet do their job** (see the honest map). They're wired, scheduled,
  and safe — but not producing output.
