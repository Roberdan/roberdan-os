...
Each of these shipped a false "done" **while every single claim was true**: they are structural.
...
- **Whole-system, not the part you touched.** Verified A but not B/C → NOT done; say exactly
  what is verified and what isn't. Partial truth stated as total is a false done.
- **"Should / probably / I think it's fine" ≠ "is".** An inference is never a verified fact:
  say "not verified yet" and go check — or say plainly you can't.
...
- **The lever is verification + gates, not model temperature.** A cold model states falsehoods
  just as confidently; reliability comes from checking before claiming, never from a sampling knob.

*(scar 2026-07-06 — "released" claimed on a push while CI was red: `rules/scars.md` § Released-on-a-push)*
...
The four shapes it catches (field read ≠ written, orphan wrapper, never ran, flag never branched on): `rules/scars.md` § Unwired.
...
Context is finite with diminishing returns ("context rot"): every always-loaded token competes
with the tokens that matter (Anthropic, effective context engineering 2025-09).
...
- **Always-loaded instruction files stay lean** — target ≤200 lines each. Remove any line whose
  ...
  ChatGPT/web bundle includes it verbatim. Before adding here, move a reason to `rules/scars.md`
  or a mechanic to a skill — never make room by dropping a rule.
...
- **Delegate by default — the orchestrator keeps conclusions, not transcripts.** Reading to find
  ...
  EARLY — a fresh window reasons better than a full one (context rot: accuracy falls as context
  grows), and past roughly half the window the next exploration goes out by default (if it must
  stay in, say why in one line). **Not delegated:** a lookup whose file and symbol you already know; a step
...
- **Subagents default to the cheap/mid class; frontier needs a written reason** (the biggest cost
  lever): the orchestrator decomposes and evaluates on the strong model, **executors** (well-scoped work with
...
- **Durable state on disk beats in-conversation state — for cost too.** A card/checkpoint is read
  once per resume; conversation state is re-paid every turn. Prefer CLIs (`gh`, `kb`) over API dumps.
- **A runaway loop is a cost incident first.** Two consecutive passes with no progress → stop and surface
  what's wedged (loop-protocol); meter long jobs against their terminal condition, never "keep trying".
...
+ branch + PR. Never run two agents/sessions committing to the same working checkout.** *(scar
2026-07-07 — two sessions on one checkout: `rules/scars.md` § Blanket-add)*

- **`git add -A` is a loaded gun whenever anything else can write to the checkout. Stage by
  explicit path.** A blanket `git add` stages *whatever is in the tree right now*, including
  another process's in-flight edits. **A `docs(...)` commit must never contain source**, and
  **mutation testing only ever in a throwaway worktree**. *(scar 2026-07-14 — a safety
...
- **Sequential work on a personal repo may still commit to the base branch directly.** The
  worktree+PR rule triggers the moment there is more than one writer.
...
ToxicSkills 2026-02: 36.8% of marketplace skills flawed, 76 malicious; an MCP server can inject
through tool *descriptions* alone). Rules:
...
goes **outside the working tree** — `~/.<repo>-env-backups/`, `chmod 600` — never `.env.bak` or
`.env.copy` next to the original. After any such copy, `git check-ignore -q <file>` must succeed
or the file does not belong there. Widening `.gitignore` is not the fix — it protects one repo, one machine, the
patterns someone thought of. *(scar 2026-08-20: `rules/scars.md` § Secret-backup)*

## Subagent models — resolved from the registry, never typed from memory

When you spawn a subagent you **must** pass `model` explicitly: the **current row of the class
you chose**, resolved through `bin/models.sh` from the reviewed registry
`skills/model-selection-policy/models.tsv` (`bin/models.sh resolve opus`; `--host claude` for the
tier alias). Never leave it to the default and never type a version number from memory. An id the
registry has not reviewed does not resolve and costs a written rationale like `frontier`; if a
host lists a generation the registry lacks, **add a reviewed row** instead of typing the id.
Which class, and the registry mechanics: the `model-selection-policy` skill. The rule here is only
*how new*, not *how big*: if unsure between two generations, **go up**. *(scar 2026-08-28 — a
verification pass redone on an id typed from memory: `rules/scars.md` § Stale-model-id)*
...
How to do a thing you already chose to do never belongs in the canon: code style, test/mock
boundaries, API conventions, local CI before push, merge discipline, review comments, repo settings
& git hooks · **Carded End-to-End** (gate: `kb cover <plan.md>` in `test/validate.sh`) · docs budget.
