...
# This assertion is the one that caught the platform bug in hooks/bus-doorbell.sh:
# it passed on macOS and failed on ubuntu-latest, because `stat -f` means "format"
# to BSD and "report on the FILESYSTEM" to GNU coreutils — and GNU exits 0, so the
# `stat -f ... || stat -c ...` fallback never fired and the signature stopped
# tracking per-file mtime. Worth stating here: an attempt to reproduce it on macOS
# with a hand-written GNU-like `stat` shim did NOT reproduce it, so the shim proved
# nothing. The CI run on real Linux is the evidence, not a local imitation of it.
