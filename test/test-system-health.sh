...
# === T3b: probe rows are private (never committed) — a missing file is "not configured", never
# a red. Uses the REAL bin/gbrain-search-probe.sh (not a stub): this is the one thing worth
# testing end to end, since it's the actual contract the private file must satisfy.
report_noprobe="$(PATH="$FAKEBIN:$PATH" RDA_HOME="$TMP/rda-home-noprobe" \
  RDA_COPILOT_STORE="$TMP/no-such.db" RDA_CLAUDE_PROJECTS="$TMP/no-such-dir" \
  RDA_HEALTH_KB="$KB_STUB_GREEN" RDA_HEALTH_PROBE_TIMEOUT=10 GBRAIN_PROBE_FILE="$TMP/no-such-probe.tsv" \
  RDA_HEALTH_TELEMETRY_CMD="$TELEMETRY_STUB" RDA_CLAUDE_HOME="$CLAUDE_HOME" \
  RDA_HEALTH_AGENTS_MD="$AGENTS_FIXTURE" \
  bash "$ROOT/bin/system-health.sh" 2>&1)" || fail "system-health.sh must exit 0 when the probe is unconfigured"
grep -q 'probe non configurata' <<<"$report_noprobe" || fail "a missing GBRAIN_PROBE_FILE must read as 'non configurata', not a failure"
grep -q 'Nessuna proposta' <<<"$report_noprobe" || fail "an unconfigured probe must not propose anything (it never ran)"

echo "PASS(3b/4): a missing private probe file reads as 'non configurata', proposes nothing"
