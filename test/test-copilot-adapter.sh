...
# The doorbell must run only after tools that can change bus/repo state (Claude parity:
# PostToolUse matcher "Bash|Edit|Write", not "*") — a read-only turn must stay silent.
...
// A READ-ONLY tool first: proves the doorbell is now gated behind the write/shell filter.
await cfg.hooks.onPostToolUse({ toolName: "view", toolArgs: { path: "/tmp/x" }, workingDirectory: process.cwd() });
await new Promise((r) => setTimeout(r, 300));
// Then a SHELL tool: proves it still rings for the tools that can actually change state.
...
# Exactly one ring expected: the "view" call above must have stayed silent and only the
# "bash" call must have rung — not "any ring at all", or a silently-broken read-only gate
# would still pass as long as the shell call rang too.
rings="$(wc -l < "$BD" | tr -d ' ')"
[ "$rings" = "1" ] && ok "bus-doorbell.sh runs on a shell tool, stays silent on a read-only tool (Claude PostToolUse matcher 'Bash|Edit|Write')" \
...
// "bash", not "view": bus-doorbell.sh only fires on write/shell tools now (matches Claude's
// PostToolUse matcher "Bash|Edit|Write"), so a read-only tool here would never reach it.
await cfg.hooks.onPostToolUse({ toolName: "bash", toolArgs: { command: "true" }, workingDirectory: process.cwd() });
