...
for _s in test-canon-guardrails test-factory-kb test-kb-views test-kb-board test-kb-done-gate test-kb-diet test-kb-queue \
          ...
          test-evolve-declined test-evolve-watch test-review-budget test-bus test-bus-mcp test-bus-doorbell test-bus-presence test-bus-owed test-bus-mutant-probes test-bus-trust test-bus-brakes test-bus-taint-guard test-bus-chat test-telemetry test-telemetry-skills test-telemetry-history test-skill-routing test-bash-guard test-factory-guard test-factory-shim test-factory-engine test-main-guard test-context-inject-staleness test-validate-wiring test-evolve-sources test-kb-autothor \
...
for _t in test-telemetry test-telemetry-skills test-telemetry-history test-skill-routing; do if _suite "$_t"; then ok "$_t: controlli superati"; else _suite_out "$_t"; err "$_t failed"; fi; done; unset _t
