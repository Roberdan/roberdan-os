...
EOF
  ...
  # Required routes carry full guidance; all wrappers preserve side-effect opt-outs.
...
EOF
    case "$name" in film-director|apple-designer)
      awk '/^---$/ && n<2 {n++; next} n>=2' "$s" >> "$d/skills/$name/SKILL.md" ;;
    esac
  done

  emit_portable_twin "$d"
...
{ "promptCacheTtl": "1h", "subagentPromptCacheTtl": "5m",
  "hooks": {
    ...
    "PostToolUseFailure": [{ "matcher": "*", "hooks": [{ "type": "command", "command": "bash $RDA_OS/hooks/audit.sh", "timeout": 5 }] }],
    "SubagentStart": [{ "matcher": "*", "hooks": [{ "type": "command", "command": "bash $RDA_OS/hooks/audit.sh", "timeout": 5 }] }],
    "SubagentStop": [{ "matcher": "*", "hooks": [{ "type": "command", "command": "bash $RDA_OS/hooks/audit.sh", "timeout": 5 }] }],
    "PreCompact": [
